...
public class ConfigHandler {
    ...
    /** Whether to use TLS for the status API. */
    public static boolean linkingApiTls;
    /** The IP or FQDN of the remote host for the status API. */
    public static String linkingApiHost;
    /** The remote port for the status API. */
    public static int linkingApiPort;
    /** The remote basepath for the status API. */
    public static String linkingApiPath;
    /** The token for the status API. */
    public static String linkingApiToken;
    /** The discord command to request a link. */
    public static String linkingCommandDiscord;
    /** The required permission level for OPs to execute `/discord status &lt;player&gt;` */
    public static int linkingSeeOtherStatusPermissionLvl;
    ...
    public static void init(File mainConfig) {
        ...
        category = createModule( ModuleStatus.KEY, "This module sends updates to a Discord bot which displays the current server status.\nAPI Version: " + StatusApi.VERSION + "\nSee https://github.com/pr0LebenImHolz/MinecraftServerStatusBot", true);
        ...
        category = createModule(ModuleLinking.KEY, "This module enables the player to link his Discord and Minecraft account", true);
        linkingApiTls = config.getBoolean("api_tls", category, true, "Enables Top Level Security (https - highly recommended)");
        linkingApiHost = config.getString("api_host", category, "", "The FQDN or IP-Address of the API (e.g. example.com)");
        linkingApiPort = config.getInt("api_port", category, 443, 0, 65535, "The port of the API");
        linkingApiPath = config.getString("api_path", category, "/", "The path of the API");
        linkingApiToken = config.getString("api_token", category, "", "The token for the API");
        linkingCommandDiscord = config.getString("discord_link_cmd", category, "/discord link %s", "The command to execute on Discord to request a link. '%s' will be replaced with the executing player's name.");
        linkingSeeOtherStatusPermissionLvl = config.getInt("discord_status_others_perm_lvl", category, 4, 0, 4, "The required OP permission level to see other player's linking status. USE WITH CAUTION!");
        linkingInfo = new FallbackEntry<>(
                // TODO: 28.03.22 add real messages
                config.getString("info_fallback_content", category, "Test Fallback", "The fallback info to display when entered /discord (should be in english)"),
                config.getString("info_native_content", category, "Test Native", "The default info to display when entered /discord (should be in the language most players are using"),
        ...
        category = createModule(ModuleRanks.KEY, "This module updates ranks (FTB-Utilities) based on playing time. When the module '" + ModuleLinking.KEY + "' is enabled, ranks will be synchronized between Discord and Minecraft.", true);
